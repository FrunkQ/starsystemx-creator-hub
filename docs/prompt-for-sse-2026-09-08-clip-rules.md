...
## Seven clips to test with, and what each should do

`docs/clips/` in the hub repo, with a README giving the expected outcome for each. They are
**generated by the hub's own builder** (`tests/clipFixtures.test.ts`) from real nodes out of the
engine's own export fixture, not typed by hand — a paste target built against a guessed fixture
passes its own tests and fails on the first real clipboard.

They cover: a body that needs a custom liquid; the same clip again (the duplicate case); the same
rules with their keys in a different ORDER (the trap); a different liquid under the same name (the
conflict); a rules-only clip with no objects; a clip with no rules at all (the regression check);
and a clip carrying a key this hub has never heard of.
